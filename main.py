...
def create_cards(message):
    ...
    # Получаем случайное слово из таблицы words
    ...
    if result:
        ...
        bot.send_message(cid, """ Ты удалил все слова, добавь новые или начни заново
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        if passed_word:
            for btn in buttons:
                if btn.text == text:
                    btn.text = text + '✅'
            hint_text = ["Отлично!❤️", show_target(data)]
            ...
            for btn in buttons:
                if btn.text == text:
                    btn.text = text + '❌'
                    break
            hint = show_hint("Допущена ошибка!", f"Попробуй ещё раз вспомнить слово 🇷🇺{translate_word}")
